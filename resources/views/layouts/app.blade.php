                ...
                <header class="sticky top-0 z-20 bg-white/80 backdrop-blur-lg border-b border-slate-200/60">
                    <div class="flex items-center justify-between px-6 py-3.5">
                        ...
                        <div class="flex items-center gap-3">
                            {{-- Date --}}
                            <span class="hidden md:block text-xs text-slate-400 font-medium">{{ now()->translatedFormat('l, d F Y') }}</span>
                            ...
                            <div x-data="{ notifOpen: false }" class="relative">
                                ...
                                <div x-show="notifOpen" x-transition x-cloak
                                    ...
                                    <div class="px-5 py-3.5 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                                        ...
                                        <a href="{{ route('notifications.index') }}" class="text-xs text-emerald-600 hover:text-emerald-700 font-semibold">Lihat semua</a>
                                    </div>
                                    <div class="max-h-80 overflow-y-auto">
                                        @forelse(auth()->user()->notifications->take(5) as $notification)
                                            <div class="px-5 py-3.5 border-b border-slate-50 {{ $notification->read_at ? 'bg-white' : 'bg-emerald-50/40' }} hover:bg-slate-50/80 transition-colors">
                                                <div class="flex items-start gap-3">
                                                    ...
                                                    <div class="flex-1 min-w-0">
                                                        <p class="text-sm font-semibold text-slate-700 truncate">{{ $notification->data['title'] }}</p>
                                                        <p class="text-xs text-slate-500 truncate mt-0.5">{{ $notification->data['message'] }}</p>
                            ...
                            <div x-data="{ userOpen: false }" class="relative">
                                <button @click="userOpen = !userOpen" @click.outside="userOpen = false"
                                    ...
                                    <div class="text-right hidden sm:block">
                                        ...
                                        <div class="text-xs text-slate-400">{{ Auth::user()->roles->pluck('name')->first() }}</div>
                                    </div>
                                ...
                                <div x-show="userOpen" x-transition x-cloak
                                    ...
                                    <a href="{{ route('profile.edit') }}" class="flex items-center gap-3 px-4 py-2.5 text-sm text-slate-600 hover:bg-slate-50 transition-colors">
