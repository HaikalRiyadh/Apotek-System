    ...
    <div class="max-w-7xl mx-auto space-y-6 animate-fade-in">
        ...
        <div class="card">
            <div class="card-body !py-3">
                <form method="GET" action="{{ route('suppliers.index') }}" class="flex gap-3 items-center">
                    ...
                    @if(request('search'))
                        <a href="{{ route('suppliers.index') }}" class="btn-secondary !py-2">Reset</a>
                    @endif
                </form>
            </div>
        </div>
        ...
        <div class="card">
                ...
                <table class="pro-table">
                    ...
                    <tbody>
                        @forelse ($suppliers as $supplier)
                            <tr>
                                <td class="font-semibold text-slate-700">{{ $supplier->name }}</td>
                                <td class="text-slate-500">{{ $supplier->phone ?? '-' }}</td>
                                <td class="text-slate-500">{{ $supplier->email ?? '-' }}</td>
                                <td class="text-slate-500">{{ $supplier->contact_person ?? '-' }}</td>
                                <td class="text-center"><span class="badge badge-info">{{ $supplier->purchases_count }}</span></td>
            ...
            <div class="px-6 py-4 border-t border-slate-100">{{ $suppliers->withQueryString()->links() }}</div>
            @endif
        </div>
    </div>
</x-app-layout>
