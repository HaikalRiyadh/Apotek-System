<section class="space-y-6">
    <header>
        ...
        <p class="mt-1 text-sm text-slate-500">
            {{ __('Setelah akun Anda dihapus, semua data akan dihapus secara permanen. Sebelum menghapus akun, silakan unduh data yang ingin Anda simpan.') }}
        </p>
    </header>
    ...
    <x-modal name="confirm-user-deletion" :show="$errors->userDeletion->isNotEmpty()" focusable>
        <form method="post" action="{{ route('profile.destroy') }}" class="p-6">
            ...
            <p class="mt-1 text-sm text-slate-500">
                {{ __('Setelah akun dihapus, semua data akan hilang permanen. Masukkan password Anda untuk mengkonfirmasi penghapusan.') }}
            </p>
